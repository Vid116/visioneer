...
| **Storage** | SQLite + in-memory vector store | Hybrid structured + vector storage, no native deps |
| **Embeddings** | Configurable (default: text-embedding-3-large) | Best quality, swappable via config |
| **Vector Search** | In-memory cosine similarity | Pure JS, persisted to SQLite, fast for <10k chunks |

### 2.1 Why This Stack

- **SQLite**: Single file, no server, portable, battle-tested
- **In-memory vector store**: Pure JavaScript cosine similarity with SQLite persistence. No native compilation issues. Scales to ~10k chunks easily, can swap to dedicated vector DB later if needed.
...
├── visioneer.db                 # SQLite database (created on init)
│
├── docs/
│   └── visioneer-technical-specification.md
│
├── src/
...
│   │   └── vector-store.ts      # In-memory vector store with SQLite persistence ✅
│   │
...
│   │   ├── prioritization.ts    # Task prioritization ✅
│   │   └── execution.ts         # Execution loop ✅
│   │
│   ├── retrieval/               # Retrieval system
│   │   └── planner.ts           # Query planner (5 query types) ✅
│   │
│   └── utils/                   # Utilities
│       ├── config.ts            # Config loading ✅
│       ├── logger.ts            # Logging (pino-based) ✅
│       └── types.ts             # Shared types ✅
│
├── src/                         # Test files (inline for now)
│   ├── test-basic-flow.ts       # Phase 1 test
│   ├── test-semantic-search.ts  # Phase 2a test
│   ├── test-coretrieval.ts      # Phase 2b test
│   ├── test-query-planner.ts    # Phase 2c test
│   └── test-agent-loop.ts       # Phase 3 test
│
└── NOT YET IMPLEMENTED:
    ├── src/agent/command.ts           # Command layer logic
    ├── src/agent/vision.ts            # Vision layer logic
    ├── src/agent/prompts/             # System prompts
    ├── src/orientation/               # Orientation management
    │   ├── triggers.ts
    │   ├── rewrite.ts
    │   └── compression.ts
    ├── src/embedding/voyage.ts        # Voyage provider
    └── src/embedding/ollama.ts        # Ollama provider
```

**Legend:** ✅ = Implemented and tested
...
- [x] In-memory vector store (replaced sqlite-vss due to Windows compatibility)
- [x] Basic MCP server structure (all three servers)
- [x] Embedding provider interface + OpenAI implementation
- [x] Mock embedding provider for testing without API
- [x] Configuration loading with environment overrides
- [x] Basic CRUD operations for all schemas

**Deliverable:** Can store and retrieve chunks, tasks, questions via MCP ✅

**Implementation Notes:**
- Vector search uses pure JavaScript cosine similarity instead of sqlite-vss
- Embeddings persisted to SQLite `chunk_embeddings_store` table
- Mock provider available via `VISIONEER_EMBEDDING_PROVIDER=mock`

### Phase 2: Memory System ✅ COMPLETE

**Goal:** Full memory system operational

- [x] Semantic search working (in-memory cosine similarity)
- [x] Tag-based filtering
- [x] Relationship creation and traversal
- [x] Implicit relationship strengthening (co-retrieval tracking)
- [x] Orientation read/write
- [x] Query planner implementation (5 query types)

**Deliverable:** Can search, relate, and retrieve knowledge intelligently ✅

**Implementation Notes:**
- Query planner routes to: operational, lookup, exploration, connection, hybrid
- Co-retrieval tracking creates implicit "related_to" relationships
- `processImplicitRelationships()` should be called periodically

### Phase 3: Agent Loop ✅ COMPLETE

**Goal:** Basic autonomous operation

- [x] Wake-up flow implementation
- [x] Task prioritization (5 factors: priority, dependencies, balance, phase, age)
- [x] Work execution loop with result handling
- [x] Question handling (create, answer, unblock)
- [x] Activity logging
- [x] Orientation rewrite triggers (milestone detection)

**Deliverable:** Agent can wake up, do work, and maintain state ✅

**Implementation Notes:**
- `TaskExecutor` is a function type - actual execution requires Claude agent
- Mock executors provided for testing
- Orientation rewrite detection implemented, but rewrite logic is Phase 4
...
- [ ] Actual Claude agent integration
...
-- Vector embeddings store (replaces sqlite-vss)
...
CREATE INDEX idx_embeddings_project ON chunk_embeddings_store(project_id);
