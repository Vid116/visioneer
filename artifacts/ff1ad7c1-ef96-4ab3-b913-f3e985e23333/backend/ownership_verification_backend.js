...
class OwnershipVerificationService {
    ...
    async getUserTilesByAddress(address) {
        try {
            ...
            for (let i = 0; i < balance; i++) {
                ...
                const position = this.tileIdToCoordinates(tileId.toNumber());
                tiles.push({
                    tileId: tileId.toNumber(),
                    x: position.x,
                    y: position.y
                });
            }
    ...
    // Send color update to hardware (placeholder for actual hardware communication)
    async sendColorToHardware(colorData) {
        // This would integrate with your existing hardware communication system
        // For now, returning a mock response
        return {
            success: true,
            timestamp: Date.now(),
            hardware_response: 'Color updated successfully'
        };
    }
...
const config = {
    ethereumRpcUrl: process.env.ETHEREUM_RPC_URL || 'https://your-ethereum-node.com',
    contractAddress: process.env.CONTRACT_ADDRESS || '0x...',
    contractAbi: [], // Your ERC-721 contract ABI
